...
- **Red** — add the next requirement as one test; watch it fail for the reason
  you expect.
- **Green** — simplest thing that passes, however blunt.
- **Refactor** — clean up code *and* tests while green. Do not skip this step
  here: requirements 5, 6 and 9 each punish a design you did not tidy.
...
- **Requirement 6** splits the input into a *header* and a *body*. Notice how
...
- Time-box it: the full list should fit in 30 minutes once the kata is familiar.
- Try it with a strict two-minute rule — if a red test is not green within two
  minutes, revert and take a smaller step.
- Re-run it later using only property tests (order of numbers never changes the
  sum, any delimiter set yields the same total as the comma form).
